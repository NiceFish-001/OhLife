...
@Component({
  selector: 'app-alldiarypicture',
  templateUrl: './alldiarypicture.component.html',
  styleUrls: ['./alldiarypicture.component.css']
})
export class AlldiarypictureComponent implements OnInit {
  ...
  diarypage(index:any,size:any){
    this.http.post(this.urlsss,{userID:this.userID,diaryindex:index,diarysize:size,diarytype:2}).subscribe((msg:any)=>{
  ...
  diaryortime(index:any,size:any,selecttime:any){
    this.http.post(this.urlsss+'/diaryortime',{userID:this.userID,diaryindex:index,diarysize:size,diarytype:2,diarytime:selecttime}).subscribe((msg:any)=>{
      if(msg.rows.length>0){
