...
@Component({
  selector: 'app-alldiaryaudio',
  templateUrl: './alldiaryaudio.component.html',
  styleUrls: ['./alldiaryaudio.component.css']
})
export class AlldiaryaudioComponent implements OnInit {
 ...
 diarypage(index:any,size:any){
   this.http.post(this.urlsss,{userID:this.userID,diaryindex:index,diarysize:size,diarytype:3}).subscribe((msg:any)=>{
 ...
 diaryortime(index:any,size:any,selecttime:any){
   this.http.post(this.urlsss+'/diaryortime',{userID:this.userID,diaryindex:index,diarysize:size,diarytype:3,diarytime:selecttime}).subscribe((msg:any)=>{
     if(msg.rows.length>0){
       this.alldiary=msg.rows;
       this.nzTotal=msg.nzTotal;
     } 
